...
- **Git & PR Automation:** Unless in coach/review mode, execute Git operations in this sequence:
  ...
  3. Push & PR: When complete, check for an existing PR using `unset GITHUB_TOKEN && gh pr view`. If a PR already exists, push commits with `git push` and update it with `unset GITHUB_TOKEN && gh pr edit` if metadata needs updating; otherwise, push with `git push -u origin HEAD` and create a new PR with `unset GITHUB_TOKEN && gh pr create --fill --body "<detailed description of changes>"` (replace with a markdown summary of what was accomplished).
