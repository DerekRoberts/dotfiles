...
- TWO-PHASE AUDIT & REPORTING: NEVER declare code, PR status, build health, or search completion clean without executing inspection tools (`view_file`, `grep_search`, `run_command`) in the active turn. For repo-wide searches/refactors, top-level recursive inspection (`grep -rn "."`) is mandatory across all directories before claiming completion. All diagnostic claims MUST be accompanied by explicit code receipts (line numbers and file snippets).
