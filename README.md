...
2. **Prompt-Scope Fencing & Behavioral Instructions (`config/instructions.md`)**: Injected into `~/.gemini/GEMINI.md` and Cursor's always-apply user rule (`~/.cursor/rules/dotfiles.mdc`) to enforce operational discipline, fail-fast mechanics, inspect-before-verify, and immutable data safety rules.
