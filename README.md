...
2. **Prompt-Scope Fencing & Behavioral Instructions (`config/instructions.md`)**: Copied to `~/.gemini/GEMINI.md`. For Cursor, setup writes `~/.cursor/rules/dotfiles.mdc` and symlinks it into each `~/Repos/*/.cursor/rules/` so the project-rule scanner picks it up (Settings User Rules are account-synced and not a local file).
