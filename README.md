...
- **Profiles**:
  - `--dev` (default): Full developer workstation (Node LTS via NVM, `gh`, `jq`, `gitleaks`, `shellcheck`, `podman-compose`, `oc`, Cursor, Antigravity, Ponytail, repositories).
...
| Node.js LTS (via NVM) | ❌ | ✅ |
| CLI Tools (`gh`, `jq`, `gitleaks`, `shellcheck`, `podman-compose`, `oc`) | ❌ | ✅ |
